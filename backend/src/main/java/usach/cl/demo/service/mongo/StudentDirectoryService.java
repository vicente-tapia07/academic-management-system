...
public class StudentDirectoryService {
    ...
    public List<StudentDirectoryItem> listStudents() {
        try {
            // _id de certificados_notas es el studentId (así lo agrupa el $group
            // de CertificateChangeStreamService).
            Set<ObjectId> withCertificate = new HashSet<>(
                    certificates.distinct("_id", ObjectId.class).into(new ArrayList<>())
            );
