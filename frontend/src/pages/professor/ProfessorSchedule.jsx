...
export default function ProfessorSchedule() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // Buscar el professor vinculado al usuario
        ...
        // Traer secciones del semestre activo
        const secRes = await api.get(`/api/sections/professor/${me.id}/active`);
  ...
  return (
    <div className="container-fluid py-4" style={{ maxWidth: 1100 }}>
      <div className="d-flex align-items-center gap-3 mb-4">
        <button className="btn btn-sm btn-outline-secondary" onClick={() => navigate('/professor')}>
          ← Volver
        </button>
        <div>
          <h2 className="fw-bold mb-0">Mi Horario</h2>
          <p className="text-muted mb-0 small">Semestre activo · Bloques oficiales USACH</p>
        </div>
      </div>
            ...
            <table className="table table-bordered mb-0" style={{ tableLayout: 'fixed', minWidth: 700 }}>
              <thead>
                <tr style={{ backgroundColor: '#003366', color: '#fff' }}>
                  <th style={{ width: 110, textAlign: 'center', verticalAlign: 'middle' }}>Bloque</th>
                  {WEEK_DAYS.map((d) => {
                    ...
                    return (
                      <th key={d.index} style={{
                        ...
                        {d.name}
                        {isToday && (
                          <span className="badge bg-warning text-dark ms-1" style={{ fontSize: '0.65rem' }}>Hoy</span>
                        )}
                      </th>
                    );
                  })}
              ...
              <tbody>
                {BLOCKS.map((block, blockIdx) => (
                  <tr key={blockIdx}>
                    <td style={{
                      textAlign: 'center', verticalAlign: 'middle',
                      backgroundColor: '#f8f9fa', fontSize: '0.78rem',
                      lineHeight: 1.3, padding: '6px 4px',
                    }}>
                      <div className="fw-bold">{block.label}</div>
                      <div className="text-muted" style={{ fontSize: '0.72rem' }}>{block.start}</div>
                      <div className="text-muted" style={{ fontSize: '0.72rem' }}>{block.end}</div>
                    </td>
                    {WEEK_DAYS.map((day) => {
                      ...
                      return (
                        <td key={day.index} style={{
                          padding: 4, verticalAlign: 'middle',
                          ...
                          {course ? (
                            <div style={{
                              ...
                              <div style={{ fontSize: '0.7rem', opacity: 0.9 }}>{course.subjectName}</div>
                              <div style={{ fontSize: '0.68rem', opacity: 0.8 }}>🚪 {course.roomName}</div>
                            </div>
                          ) : (
                            <div style={{
                              height: '100%', display: 'flex',
                              alignItems: 'center', justifyContent: 'center',
                              color: '#dee2e6', fontSize: '1rem',
                            }}>—</div>
                          )}
                        </td>
                      );
                  ...
                  {[...courses]
                    ...
                    .map((c) => {
                      ...
                      return (
                        <tr key={c.id}>
                          <td>
                            <span className="badge me-1" style={{ backgroundColor: c.color }}>{c.subjectCode}</span>
                          ...
                          <td className="text-center text-muted small">
                            {c.totalSeats - c.availableSeats} / {c.totalSeats}
                          </td>
                        </tr>
                      );
                    })}
