...
export default function StudentEnrollAdmin() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [studRes, secRes, subRes, semRes] = await Promise.all([
          api.get("/api/students"),
          api.get("/api/sections"),
          api.get("/api/subjects"),
          api.get("/api/semesters"), // Traemos los semestres
        ]);

        // Buscamos el semestre activo
        const activeSemester = semRes.data.find(
        ...
        // Filtramos secciones: que tengan cupos Y que pertenezcan al semestre activo
        if (activeSemester) {
          const activeSections = secRes.data.filter(
            (s) => s.availableSeats > 0 && s.semesterId === activeSemester.id,
          );
          setSections(activeSections);
        } else {
          setSections([]); // Si no hay semestre activo, no hay secciones donde inscribir
        }
      } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes("prerequisit")) {
        ...
        msg.toLowerCase().includes("enrollment_student")
      ) {
      ...
      <div className="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-muted mb-0">
            El admin inscribe a un estudiante en una sección
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              ...
              >
                ...
                {sections.length === 0 && (
                  <option disabled>No hay secciones activas con cupos</option>
                )}
                {sections.map((s) => (
                  <option key={s.id} value={s.id}>
                    {subjectCode(s.subjectId)} · {subjectName(s.subjectId)} —
                    Sección #{s.id} ({s.availableSeats} cupos)
                  </option>
                ))}
              </select>
            </div>
            ...
            {seccionSeleccionada && (
              <div className="alert alert-info py-2 small mb-3">
                ...
                {" · "}Cupos disponibles: {seccionSeleccionada.availableSeats}/
                {seccionSeleccionada.totalSeats}
              </div>
            )}
